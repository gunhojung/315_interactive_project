...
*  From previous analysis, we investigated how reliance on Situational Strategy differs by the regular season ranking. Now, we are trying to see if there is a correlation between Situational Strategy and the number of teams qualified for the play-offs. According to the graph, there is no significant difference between American League and National League in terms of play-off qualification as the confidence band overlaps greatly. Also, there is no pattern between these two variables, which indicate that teams that are qualified for the play-offs tend or tend not to heavily rely on the implementation of Situational Strategy. Thus, it is interesting that there is no correlation between the number of times teams were qualified for the play-offs and their reliance on Situational Strategy.
